...
class BaseScraper:
    ...
    def get_metadata(self) -> any:
        ...
        return getattr(self.crawl_result, "page_metadata", None)
        ...
        if thumbnail:
            ...
            if screenshot:
                thumbnail_path = os.path.join(
                    output_dir, f"{base_filename}.png"
                )
    ...
    async def bulk_crawl(
        ...
        try:
            async with self.crawler as crawler:
                ...
                if session_id:
                    ...
                    config = dataclasses.replace(
                        self.run_config, session_id=session_id
                    )
                ...
                if session_id:
                    self.crawl_results = []
                    for url in urls:
                        url_config = dataclasses.replace(config, url=url)
                ...
                logger.info(
                    "Bulk crawl completed with"
                    f" {len(self.crawl_results)} results"
                )
                ...
                if session_id:
                    try:
                        ...
                        logger.warning(
                            f"Failed to clean up session {session_id}:"
                            f" {str(e)}"
                        )

                # Store last result in crawl_result for backwards compatibility
                self.crawl_result = (
                    self.crawl_results[-1] if self.crawl_results else None
                )
            ...
            if session_id:
                try:
                    await self.crawler.crawler_strategy.kill_session(
                        session_id
                    )
                except Exception as cleanup_error:
                    logger.warning(
                        "Failed to clean up session after error:"
                        f" {str(cleanup_error)}"
                    )
            return []
    ...
    async def save_bulk_results(
        ...
        try:
            ...
            summary_file = os.path.join(
                self.output_dir, f"kb_articles_summary_{timestamp}.json"
            )
...
class MicrosoftKbScraper(BaseScraper):
    ...
    def __init__(
        ...
        if extraction_method.lower() == "llm":
            ...
            prompt = (
                "Extract a structured JSON object from the following HTML"
                " that represents a Microsoft KB report. The JSON object must"
                " conform to the following"
                f" schema:\n\n{json.dumps(json_schema, indent=4)}\n\nEnsure"
                " that each field is extracted correctly from the HTML."
        ...
        kb_run_config = CrawlerRunConfig(
            ...
            excluded_selector=(
                ...
                " .ocArticleFooterFeedPickerContainer,"
                " .ocArticleFooterSection.articleSupportBridge,"
                " #ocFooterWrapper"
            ),
            verbose=True,
        ...
        self.bulk_crawler = AsyncWebCrawler(
            config=kb_browser_config, dispatcher=bulk_dispatcher
        )
    ...
    def _update_rate_limiter(self, max_retries: Optional[int]) -> None:
        ...
        Args:
            max_retries (Optional[int]): New maximum number of retry attempts. If None,
        ...
        if (
            hasattr(self, 'bulk_crawler')
            and self.bulk_crawler.dispatcher.rate_limiter
        ):
    ...
    async def save_kb_bulk_results(
        self, results: List[Any], output_dir: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
                ...
                try:
                    ...
                    if content:
                        if isinstance(content, list):
                            ...
                            for item in content:
                                if isinstance(item, dict):
                                    try:
                                        ...
                                        logger.error(
                                            "Validation error for an element"
                                            f" in {url}: {str(e)}"
                                        )
                                        continue
                                else:
                                    logger.error(
                                        f"Element in content list for {url} is"
                                        " not a mapping"
                                    )
                            if not valid_items:
                                logger.error(
                                    "No valid KBArticle objects in list for"
                                    f" {url}"
                                )
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to validate content for {url}:"
                                    f" {str(e)}"
                                )
                                error_path = os.path.join(
                                    self.output_dir,
                    ...
                    if hasattr(result, "pdf") and result.pdf:
                        pdf_path = os.path.join(
                            self.output_dir, f"{base_filename}.pdf"
                        )
                        with open(pdf_path, "wb") as f:
                            f.write(result.pdf)
                    ...
                    if hasattr(result, "raw_html"):
                        raw_path = os.path.join(
                            self.output_dir, f"{base_filename}_raw.html"
                        )
            ...
            try:
                ...
                return (
                    None if failed_saves == len(results) else self.output_dir
                )
...
class GenericWebScraper(BaseScraper):
    ...
    def __init__(
        ...
        if extraction_config:
            ...
            if strategy_type == "llm":
                ...
                extraction_strategy = LLMExtractionStrategy(
                    ...
                    apply_chunking=extraction_config.get(
                        "apply_chunking", True
                    ),
        ...
        crawler_cfg = (
            CrawlerRunConfig(
                cache_mode=crawler_config.get(
                    "cache_mode", CacheMode.DISABLED
                ),
                extraction_strategy=extraction_strategy,
                word_count_threshold=crawler_config.get(
                    "word_count_threshold", 0
                ),
                exclude_external_links=crawler_config.get(
                    "exclude_external_links", True
                ),
                wait_until=crawler_config.get(
                    "wait_until", "domcontentloaded"
                ),
        ...
        super().__init__(browser_config=browser_cfg, run_config=crawler_cfg)
        ...
        self.bulk_crawler = AsyncWebCrawler(
            config=browser_cfg, dispatcher=bulk_dispatcher
        )
        logger.info("Initialized bulk crawler with memory-adaptive dispatcher")
    ...
    async def save_results(
        ...
        try:
            ...
            summary_path = os.path.join(
                self.output_dir, f"scrape_summary_{timestamp}.json"
            )
            with open(summary_path, "w", encoding="utf-8") as f:
                json.dump(summary, f, indent=2)
